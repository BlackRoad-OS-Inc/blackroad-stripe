...
AUTH_TOKEN = os.getenv(
    "MCP_BRIDGE_TOKEN",
    "blackroad-blackroad-stripe-bridge-token",
)
MEMORY_DIR = Path.home() / "blackroad" / "memory" / "blackroad-stripe"
MEMORY_DIR.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="blackroad-stripe MCP Bridge")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/")
async def root():
    return {"service": "blackroad-stripe MCP Bridge", "host": platform.node()}
